...
import amplifierenable, cecaudio
...
def reload(parameters):
    ...
    global current_parameters, high_volume, low_volume, cec_audio_control
    """
...
def cec_callback(action):
    ...
    if action == 'vol_up' and low_volume < 10.0:
        ...
        low_volume *= 1.2
        high_volume *= 1.2
        print(f'cec volume up (lo:{low_volume:.5f} high:{high_volume:.5f})')
    elif action == 'vol_down' and low_volume > 0.0003:
        muted = False
        cec_audio_control = True
        low_volume *= 1.0 / 1.2
        high_volume *= 1.0 / 1.2
        print(f'cec volume down (lo:{low_volume:.5f} high:{high_volume:.5f})')
    elif action == 'mute':
        muted = not muted
        print(f'cec mute {muted}')
        
    for path in [Configuration.LEFT.value, Configuration.RIGHT.value]:
        gst_pipeline.get_by_name(f'low_vol{path}').set_property('volume', 0.0001 if muted else low_volume)
        gst_pipeline.get_by_name(f'high_vol{path}').set_property('volume', 0.0001 if muted else high_volume)
...
cec = cecaudio.cecaudio(cec_callback)
